    ...
    inline void vcpu_metrics(std::string &out, size_t rc, virDomainStatsRecordPtr *stats)
    {
        for (size_t j = 0; j < rc; j++)
        {
            ...
            for (int k = 0; k < record->nparams; k++)
            {
                ...
                if (fields.size() == 3)
                {
                    out.append(custom::format("# TYPE virt_%s_%s counter\n", fields[0].c_str(), fields[2].c_str()));
                    out.append(custom::format("virt_%s_%s{domain=\"%s\", vcpu=\"%s\"} %llu\n",
                                              fields[0].c_str(),
                                              fields[2].c_str(),
    ...
    inline void network_metrics(std::string &out, size_t rc, virDomainStatsRecordPtr *stats)
    {
        for (size_t j = 0; j < rc; j++)
        {
            ...
            for (int k = 0; k < record->nparams; k++)
            {
                ...
                if (fields.size() == 4)
                {
                    out.append(custom::format("# TYPE virt_%s_%s_%s counter\n", fields[0].c_str(), fields[3].c_str(), fields[2].c_str()));
                    out.append(custom::format("virt_%s_%s_%s{domain=\"%s\" interfaceid=\"%s\", name=\"%s\"} %llu\n",
